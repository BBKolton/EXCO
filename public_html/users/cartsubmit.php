...
if ($_POST['type'] == 'credit') {
	...
	if ($curlData["result"] === "0" || $_POST['type'] != 'credit') {
		...
		if (empty($curlData['Name'])) {
			$curlData["Name"] = "An Error Processing Your Card Occurred";
			$curlData["Message"] = "Please ensure your CVC code is correct, and your expiration date is in the correct format of MMYY, no slashes included.";
		} 
		error($curlData["Name"], $curlData["Message"] . "<!--" . print_r($curlData, true) . " --> This error was autogenerated. Contact EXCO if you need assistance");
	}
} else {
	registerCourses($DATABASE, $user);
	displayConfirmation(null, $DATABASE, $db);
	sendConfirmationEmail(null, $DATABASE, $user, $newUser);
	$_SESSION["cart"] = null;
}
...
function verifyUserData() {
	if ($_POST['type'] == 'credit') {
		...
		error('Insufficient Credentials', 'You are not allowed to charge other than credit');
	}
}
...
function registerCourses($DATABASE, $user) {
	...
	for ($i = 0; $i < count($_SESSION["cart"]); $i+=2) {
		...
		$db -> query("INSERT INTO " . $DATABASE . ".registrations
		             ...
		             (" . $id . ", " . $section . ", " . $db->quote($user[0]["id"]) . ", " . $type . ")");
	}
}
